...
class Go1_train_env(Legged_Robot_Torch):
    def __init__(self, Cfg, headless):
        ...
        if self.config.terrain.measure_heights:
            ...
            self.height_data = torch.from_numpy(self.height_data)  # 从numpy数组创建tensor
            self.height_data = self.height_data.to(device=self.device, dtype=torch.float32)  # 单独设置设备和类型
            measured_points_x = [
                -0.8,
                -0.7,
                -0.6,
