...
class Legged_Robot_Torch:
    ...
    def buffer_init(self):
        ...
        self.extras = {}
        self.device = self.config.sim.device
        self.dof_pos = torch.zeros((self.config.env.num_envs, self.config.env.num_actions), dtype=torch.float32)
        self.dof_vel = torch.zeros((self.config.env.num_envs, self.config.env.num_actions), dtype=torch.float32)
        self.commands = torch.zeros((self.config.env.num_envs, 3), dtype=torch.float32)
        self.obs = torch.zeros((self.config.env.num_envs, self.config.env.num_observations), dtype=torch.float32)
        ...
        self.episode_length_buf = torch.zeros(self.config.env.num_envs, dtype=torch.int32)
        self.linear_vel = torch.zeros((self.config.env.num_envs, 3), dtype=torch.float32)
        ...
        self.actions = torch.zeros((self.config.env.num_envs, self.config.env.num_actions), dtype=torch.float32)
        self.gravity = torch.zeros((self.config.env.num_envs, 3), dtype=torch.float32)
        self.rew_buf = torch.zeros(self.config.env.num_envs, device=self.device, dtype=torch.float32)
        self.last_dof_vel = torch.zeros_like(self.dof_vel)
        self.gravity_vec = torch.tensor([0, 0, -1], device=self.device, dtype=torch.float32).repeat(
        ...
        if self.config.env.num_privileged_obs is not None:
            self.privileged_obs_buf = torch.zeros(
                self.config.env.num_envs, self.config.env.num_privileged_obs, device=self.device, dtype=torch.float
            )
        else:
        ...
        self.default_angles = torch.zeros(self.config.env.num_actions, dtype=torch.float32)
        found = False
        for i in range(self.model.num_actuators):
            for name in self.config.init_state.default_joint_angles.keys():
                if name in self.model.actuator_names[i]:
                    self.default_angles[i] = self.config.init_state.default_joint_angles[name]
                    found = True
            if not found:
                self.default_angles[i] = self.config.init_state.default_joint_angles["default"]
            if not k:
                found = False
        ...
        self.last_actions = torch.zeros(self.config.env.num_envs, self.config.env.num_actions, dtype=torch.float32)
        ...
        for name in self.termination_contact_name:
            if self.termination_contact is None:
                self.termination_contact = np.array([[self.model.get_geom_index(name), self.ground]], dtype=np.uint32)
            else:
                self.termination_contact = np.append(
                    self.termination_contact,
                    np.array([[self.model.get_geom_index(name), self.ground]], dtype=np.uint32),
                    axis=0,
                )
        self.foot = None
        ...
        for i in self.geom_names:
            ...
            if self.config.asset.foot_name in i:
                if self.foot is None:
                    self.foot = np.array([[self.model.get_geom_index(i), self.ground]], dtype=np.uint32)
                else:
                    self.foot = np.append(
                        self.foot,
                        np.array([[self.model.get_geom_index(i), self.ground]], dtype=np.uint32),
                        axis=0,
                    )
        print(self.foot)
        ...
        self.termination_check = self.termination_contact#np.tile(self.termination_contact, (*batch, 1))
        ...
        self.feet_air_time = torch.zeros(
            self.num_envs, self.foot_check_num, dtype=torch.float32, device=self.device, requires_grad=False
        )
        ...
        if self.config.sensor.contact_sensor:
            ...
            self.contact_data = torch.zeros(self.num_envs, len(self.config.sensor.foot_name) , 1+12*self.config.sensor.num_contact, dtype=torch.float32, device=self.device, requires_grad=False)
            for i in range(len(self.config.sensor.foot_name)):
                    ...
                    self.contact_data[:, i, :] = torch.tensor(self.model.get_sensor_value(self.config.sensor.foot_name[i], self.datas), device=self.device, dtype=torch.float32)
    ...
    def step(self, actions):
        ...
        self.actions = torch.clip(self.actions, -self.config.control.clip_actions, self.config.control.clip_actions)
        ...
        return self.obs, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras
    ...
    def get_data(self):
        self.linear_vel = torch.from_numpy(self.model.get_sensor_value(self.config.sensor.local_linvel, self.datas))
        self.gyro = torch.from_numpy(self.model.get_sensor_value(self.config.sensor.gyro, self.datas))
    ...
    def _post_physics_step_callback(self):
        ...
        env_ids = (self.episode_length_buf % 100 == 0).nonzero(as_tuple=False).flatten()  ##### resample time in config
    ...
    def reset_dofs(self, env_ids):
        ...
        for i in range(self.config.env.num_actions):
            self.joints[i].set_dof_pos(self.datas[bool_mask], len(env_ids) * [self.default_angles[i]])
        #### add noise
    ...
    def _sync_dof_data(self):
        if self.config.sensor.contact_sensor:
            for i in range(len(self.foot_sensor_names)):
                self.contact_data[:, i, :] = torch.tensor(self.model.get_sensor_value(self.foot_sensor_names[i], self.datas), device=self.device, dtype=torch.float32)
    ...
    def _prepare_reward_function(self):
        ...
        # reward episode sums
        self.episode_sums = {
            name: torch.zeros(self.config.env.num_envs, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def _reward_tracking_lin_vel(self):
        ...
        lin_vel_error = torch.sum(torch.square(self.commands[:, :2] - self.linear_vel[:, :2]), dim=1)
    ...
    def _reward_hip_pos(self):
        ...
        return (0.8 - torch.abs(self.commands[:, 1])) * torch.sum(
            torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_angles[[0, 3, 6, 9]]),
            dim=1,
        )
    ...
    def _reward_calf_pos(self):
        # return torch.sum(torch.square(self.dof_pos[:, [0, 3, 6, 9]] - self.default_dof_pos[:, [0, 3, 6, 9]]), dim=1)
        return (0.8 - torch.abs(self.commands[:, 1])) * torch.sum(
            torch.square(self.dof_pos[:, [2, 5, 8, 11]] - self.default_angles[[2, 5, 8, 11]]),
            dim=1,
        )
