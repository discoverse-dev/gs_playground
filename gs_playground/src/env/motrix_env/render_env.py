...
class NpRenderEnv(NpEnv):
    ...
    def _compute_camera_arrays(self, data) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        for cid in self._cam_ids:
            ...
            fovys.append(np.full((cam_pose.shape[0],), float(cam.fovy), dtype=np.float32))
    ...
    def _compute_body_arrays(self, data) -> Tuple[np.ndarray, np.ndarray]:
        ...
        link_poses = self._model.get_link_poses(data)  # (B, nbody, 7): pos(3)+quat(4 xyzw)
    ...
    def batch_render(self, data) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self._bg_renderer is not None and self._bg_imgs is None:
            ...
            self._bg_imgs, _ = self._bg_renderer.batch_env_render(bg_gsb, cam_xpos, cam_xmat, H, W, fovy)
        ...
        rgb_t, depth_t = self._fg_renderer.batch_env_render(gsb, cam_xpos, cam_xmat, H, W, fovy, bg_imgs=effective_bg)
    ...
    def _reset_done_envs(self):
        ...
        if self._bg_renderer is not None:
            ...
            bg_imgs_sub, _ = self._bg_renderer.batch_env_render(bg_gsb_sub, cam_xpos_sub, cam_xmat_sub, self._img_h, self._img_w, fovy_sub)

            if self._bg_imgs is None:
                B = int(self._num_envs)
                self._bg_imgs = torch.zeros((B,) + tuple(bg_imgs_sub.shape[1:]), dtype=bg_imgs_sub.dtype, device=bg_imgs_sub.device)
