    ...
    try:
        ...
        if isinstance(mesh, trimesh.base.Trimesh):
            ...
            for i, (material_name, geom) in enumerate(mesh.geometry.items()):
                # Filter out meshes with fewer than 2 faces or fewer than 4 vertices
                if len(geom.vertices) < 4:
                    print(f"忽略子网格 (材质: {material_name})，顶点数 ({len(geom.vertices)}) 少于 4")
                    continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="将 .obj, .stl 或 .ply 网格文件转换为 MuJoCo MJCF 格式的XML文件。")
    parser.add_argument("input_file", type=str, help="输入的网格文件路径 (.obj, .stl 或 .ply)。")
    ...
    parser.add_argument("-o", "--output", type=str, default="./assets", help="输出的资产文件路径，默认为 ./assets")
    ...
    if ext in ['.obj', '.stl', '.ply']:
        asset_name = os.path.basename(input_file)[:-len(ext)]
    else:
        exit(f"错误: {input_file} 不是有效的文件类型。请使用 .obj, .stl 或 .ply 文件。")
    ...
    if os.path.dirname(input_file) != output_dir:
        ...
        if input_file.lower().endswith(".obj"):
            ...
            try:
                import trimesh
                print(f"正在将 {input_file} 转换为 OBJ 格式...")
                mesh = trimesh.load(input_file, force="mesh")
                output_path = Path(output_dir) / f"{asset_name}.obj"
                mesh.export(str(output_path))
            except ImportError:
                print("错误: 转换非 OBJ 文件需要 trimesh 库。请使用 'pip install trimesh' 安装。")
                exit(1)
    ...
    if convex_de:
        ...
        coacd_config = coacd_config_scene if args.scene else {"merge": False}
        ...
        for i, part in enumerate(parts):
            if len(part[0]) < 5:
                print(f"忽略凸分解部分 {i}，顶点数 ({len(part[0])}) 少于 5")
                continue
                
            part_filename = f"part_{valid_parts_count}.obj"
        ...
        convex_parts_count = valid_parts_count
        print(f"资源 {asset_name} 已被分解为 {convex_parts_count} 个有效的凸包部分。")
